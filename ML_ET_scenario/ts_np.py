...
def get_TS_np(features):
    ...
    full_filename = os.path.join(ML_DIR, "ML_Scenario.csv")
    scenario_df = pd.read_csv(full_filename, sep=' ', low_memory=False)
    ...
    for atco in ATCOs:
        ...
        scenario_atco_df = scenario_df[scenario_df['ATCO']==atco]
        ...
        for run in range(1,4):
            ...
            scenario_run_df = scenario_atco_df[scenario_atco_df['Run']==run]
            
            if et_run_df.empty:
                continue
        
            number_of_time_intervals = len(scenario_run_df.index)
        
            run_TS_np = np.zeros(shape=(number_of_time_intervals, window_size, number_of_features))
            print(run_TS_np.shape)
            all_run_scores = scenario_run_df['score'].tolist() 
            run_scores = []
